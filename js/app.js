                ...
                controller: ["$scope", "$timeout", "$rootScope", "$state", "$http", "$sce", '$filter', '$modal', "accountService", "domainService", "competitorService", "keywordService", "autoReportService", "usersService", "shareService", "groupSearchFactory", "config", function($scope, $timeout, $rootScope, $state, $http, $sce, $filter, $modal, accountService, domainService, competitorService, keywordService, autoReportService, usersService, shareService, groupSearchFactory,  config) {
                    ...
                    $scope.selectAllKeywords = function () {
                        ...
                        for (var i = 0; i < $scope.Domain.Keywords.length; i++) {
                            $scope.Domain.Keywords[i].Selected = $scope.bulk.allChecked;
                        }
                    };

                    /*$scope.changeEmail = function (newEmail) {

                        accountUsersService.changeEmail(newEmail).then(function (response) {

                            $scope.Domain.EmailIsFake = false;

                            $scope.changeEmailSuccessMessage = angular.fromJson(response.data);
                            $scope.changeEmailErrorMessage = null;
                        }, function (data) {
                            $scope.changeEmailErrorMessage = data.Message;
                            $scope.changeEmailSuccessMessage = null;
                        });
                    };*/
                    ...
                    $scope.emailSharePopup = function (successId) {
                        ...
                        $modal.open({
                            ...
                            resolve: {
                                successId: function () { return successId; },
                    ...
                    $scope.bulkActionChange = function () {

                        if ($scope.getSelectedKeywordIds().length == 0) {
                            $scope.hideMessages();
                            $scope.keywordErrorMessage = "No keywords selected.";
                            $scope.resetBulkAction();
                        }

                        $scope.showGroupList = false;

                        if ($scope.bulk.action == "delete") {
                            $scope.bulkDeleteKeywords();
                        }
                        else if ($scope.bulk.action == "assignGroup") {
                            $scope.showGroupList = !$scope.showGroupList;
                        }
                    };

                    $scope.resetBulkAction = function () {
                    ...
                    $scope.bulkAssignGroupToKeywords = function () {
                        ...
                        if (keywordIds.length > 0) {
                            ...
                            var modalInstance = $modal.open({
                                templateUrl: config.BASE_PATH + 'templates/Panel/Popups/assignKeywordsToGroup.html',
                                controller: 'BulkAssignGroupToKeywordsPopupCtrl',
                                resolve: {
                                    ids: function () { return keywordIds; },
                                    group: function () { return $scope.bulk.group; },
                                    config: function () { return config; }
                                }
                            });
                            ...
                            modalInstance.result.then(function () {
                                ...
                                $scope.reload(false, $scope.currentPage);
                                $scope.hideMessages();
                                $scope.keywordSuccessMessage = keywordIds.length + " keyword(s) has been assigned to " + $scope.bulk.group.Name;
                                $scope.addingKeywordLoading = false;

                                $scope.selectedAllChecked = false;
                                $scope.resetBulkAction();
                                ...
                                if (errorMessage) {
                                    $scope.hideMessages();
                                    $scope.keywordErrorMessage = errorMessage;
                                }
                            });
                        } else {
                            $scope.hideMessages();
                    ...
                    $scope.autoReportPopup = function (advanced) {

                        $modal.open({
                            templateUrl: config.BASE_PATH + 'templates/Panel/Popups/autoReport.html',
                            controller: 'AutoReportPopupCtrl',
                            windowClass: advanced ? 'w800px' : '',
                            resolve: {
                                accountDomainId: function () { return $rootScope.userDomainId; },
                                dict: function () { return $scope.dict; },
                                autoReportPopup: function () { return $scope.autoReportPopup },
...
angular.module('app').controller('BulkAssignGroupToKeywordsPopupCtrl', ['$scope', '$rootScope', '$modalInstance', 'ids', 'group', 'keywordService', 'config',
    function ($scope, $rootScope, $modalInstance, ids, group, keywordService, config) {
        ...
        $scope.ok = function () {
            ...
            keywordService.addGroupToKeywordBulk(ids, group.Name).then(
                function (response) {
                    if (response.data.success) {
                        $modalInstance.close();
                    } else {
                        $modalInstance.dismiss(response.data.error);
                    }
                }, function (data) {
...
angular.module('app').controller('SendGraphCtrl', ['$scope', '$modalInstance', 'shareService', 'usersService', 'accountDomainId', 'interval', 'filter', 'config',
    function ($scope, $modalInstance, shareService, usersService, accountDomainId, interval, filter, config) {
        ...
        $scope.send = function () {
            ...
            $scope.popupLoading = true;
            shareService.sendGraphEmail($scope.form.sendTo, $scope.form.fromName, accountDomainId, interval, filter, function () {
...
angular.module('app').controller('AutoReportPopupCtrl', ['$scope', 'accountService', 'autoReportService', '$modalInstance', 'accountDomainId', 'dict', 'autoReportPopup', 'advanced', 'config',
    function ($scope, accountService, autoReportService, $modalInstance, accountDomainId, dict, autoReportPopup, advanced, config) {
