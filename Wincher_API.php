...
Class Wincher_API {
    public static $base_url = 'https://wp.wincher.com/v7/';
    ...
    public static function processRequest($request_type) {
        if (empty($request_type)) {
            ...
            switch ($request_type) {
                ...
                case 'keywordgroup_add_bulk':
                    self::keywordGroupAddBulk();
                    break;
    ...
    public static function getKeywordChart() {
        $request_fields = array(
            ...
            "Height"            => intval($_REQUEST["Height"]),
            "GraphInterval"     => intval($_REQUEST["GraphInterval"]),
            "IncludeCompetitors"=> filter_var($_REQUEST["IncludeCompetitors"], FILTER_VALIDATE_BOOLEAN),
        );
    ...
    public static function keywordGroupAddBulk() {

        $request_fields = array(
            "accountDomainKeywordIds" => array_map('intval', $_REQUEST["accountDomainKeywordIds"]),
            "name" => $_REQUEST["name"],
        );
        $url = 'keywordgroup/addbulk';
        $method = 'POST';
    ...
    public static function setAutoreport() {
        ...
        $request_fields = array(
            ...
            "Enabled" => filter_var($_REQUEST["Enabled"], FILTER_VALIDATE_BOOLEAN),
            "Filter" => $_REQUEST["Filter"],
        );
